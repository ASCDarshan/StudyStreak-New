    ...
    <div className="p-8 animate-fade-in-up">
        ...
        {Object.entries(scores).map(([skill, value]) => (
          <div key={skill} 
            ...
            <div className="flex justify-between items-center">
              ...
              <span className={`${getScoreColor(value)} font-bold text-lg
            ...
            <input
              ...
              step="0.5"
              value={value}
              onChange={(e) => handleScoreChange(skill, parseFloat(e.target.value))}
            ...
            <div className="flex justify-between text-xs text-neutral-500">
              ...
              <div className="flex gap-1">
                {[4.5, 5, 5.5, 6, 6.5, 7, 7.5, 8, 8.5, 9].map((mark) => (
                  <div 
                    key={mark} 
                    className={`w-px h-2 ${value >= mark ? 'bg-primary-300' : 'bg-neutral-300'} 
      ...
      <div className="mt-8 pt-8 border-t border-neutral-200">
        <div className="text-center transform hover:scale-105 transition-transform duration-300">
          <p className="text-sm text-neutral-600 mb-2">Your Overall Band Score</p>
          <div className={`text-5xl font-bold ${getScoreColor(overallBand)} 
            animate-bounce-soft transition-all duration-300`}>
            {overallBand.toFixed(1)}
          </div>
          <div className="mt-4 text-sm text-neutral-500">
            {overallBand >= 8 ? '🎉 Excellent Score!' : 
             overallBand >= 7 ? '👍 Good Score!' :
             overallBand >= 6 ? '💪 Keep Improving!' : 
